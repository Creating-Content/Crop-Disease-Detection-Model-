...
def resize_image_keeping_aspect_ratio(image_pil: Image.Image, max_dim: int) -> Image.Image:
    """
    Resizes a PIL Image to have its longest side no greater than max_dim,
    maintaining the aspect ratio.
    """
    original_width, original_height = image_pil.size
    
    if max(original_width, original_height) <= max_dim:
        return image_pil # No resizing needed if already within limits
...
def segment_with_unet(image_pil: Image.Image, mask_radius_factor=0.75):
    ...
    # original_width, original_height = image_pil.size # Stored for upscaling later
    ...
    # Resize to 256x256 for consistent contour drawing base, then convert to OpenCV format
    img_np_resized_for_contours = np.array(image_pil.resize((256, 256))) 
    circled_img = cv2.cvtColor(img_np_resized_for_contours, cv2.COLOR_RGB2BGR) # Convert RGB to BGR
    ...
    # Upscale the image with drawn circles back to a reasonable display resolution
    # Instead of original_width, original_height, use a fixed MAX_DISPLAY_DIM
    # This prevents memory spikes if original image was extremely large.
    MAX_DISPLAY_DIM = 1024 # You can adjust this value
    display_width, display_height = image_pil.size # Use the *potentially resized* input image's dimensions

    # Apply aspect-ratio-preserving resize for the final output image for consistency
    if max(display_width, display_height) > MAX_DISPLAY_DIM:
        if display_width > display_height:
            new_output_width = MAX_DISPLAY_DIM
            new_output_height = int(MAX_DISPLAY_DIM * (display_height / display_width))
        else:
            new_output_height = MAX_DISPLAY_DIM
            new_output_width = int(MAX_DISPLAY_DIM * (display_width / display_height))
        upscaled_circled_img = cv2.resize(circled_img, (new_output_width, new_output_height), interpolation=cv2.INTER_LINEAR)
    else:
        # If the input image itself was already small enough, just resize to its original size
        upscaled_circled_img = cv2.resize(circled_img, (display_width, display_height), interpolation=cv2.INTER_LINEAR)
...
def analyze_plant_endpoint(): # Changed function name
    ...
    try:
        ...
        # --- NEW: Resize input image immediately to control memory footprint ---
        # Adjust MAX_INPUT_DIM as needed, 1280 is a good starting point for many photos
        MAX_INPUT_DIM = 1280 
        image_pil = resize_image_keeping_aspect_ratio(image_pil, MAX_INPUT_DIM)
    ...
    # The segment_with_unet function now handles the output image resizing internally
